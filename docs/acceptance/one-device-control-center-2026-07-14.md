...
- `make macos-test` passes 50 deterministic macOS seam contracts; earlier full-scope acceptance also passed the Android local unit-test task, `make macos-build`, `make doctor`, shell syntax checks, and `git diff --check`.
- `make macos-release VERSION=0.1.0 BUILD_NUMBER=1 ARCHITECTURE=native BUILD_SYSTEM=xcode SIGNING_IDENTITY=-` rebuilt the persistent current-source release output through the real shared Xcode scheme. Bundle verification confirmed version `0.1.0` (`1`), identifier `com.facundopri.tetherpane`, arm64 architecture, sealed metadata/resources, an ad-hoc hardened-runtime signature, and a versioned ZIP with SHA-256 `897c57cf35c97a82656feabd24a536c7bc6e7a2057f5b839fa75907d13f63ec0`.
- `make xcode-project-test` performed a real Debug build with Xcode 26.6 and ended with `** BUILD SUCCEEDED **`. `make test` passed all 50 macOS seam contracts and the Android local unit-test task.
...
- A later live run began with exactly one authorized secure Wireless Debugging endpoint and no USB ADB endpoint. The real **Mirror** button launched stock scrcpy 4.1 and its `com.genymobile.scrcpy.Server 4.1` child against that exact secure serial; **Stop Mirroring** removed the process and returned the same workspace to **Mirror**.
- **Disconnect on This Mac** then removed every local ADB device while the Android `_adb-tls-connect._tcp` service remained advertised. TetherPane moved the phone to **Offline · Disconnected on this Mac**, explained that Android trust was unchanged, and offered **Reconnect over Wi-Fi**. Reconnect restored the exact endpoint returned by `adb connect`; a second Mirror/Stop cycle targeted that returned serial exactly and left the endpoint ready.
- A fresh read-only follow-up found exactly that one authorized TLS endpoint in `adb devices -l`, the matching `_adb-tls-connect._tcp` service in mDNS, an empty `SPUSBDataType` device array from macOS, and no running `scrcpy` process. The foreground app still showed one **Wi-Fi · Connected** device with **Connected securely**. This proves no USB data device or transport was present; it does not infer that a charge-only cable was physically absent.

Final person-held-device confirmation:

- On 2026-07-14, after Xcode became available, the person holding the Motorola reported testing all TetherPane features with Android and that they worked and looked great. This is the user-reported acceptance record for the remaining hands-on cable-removal, fresh secure-pairing, and device-management paths; it is deliberately distinct from the machine-observed ADB, process, and UI evidence above.
- The app never claims a physical-device identity from ambiguous endpoints. That safety behavior is covered deterministically by the same-name, unclassified-route, and ambiguous secure-association seam contracts rather than by inventing an ambiguous physical-device setup for acceptance.

## UI inspection status

The final development `.app` built and launched in the foreground through `./script/build_and_run.sh --verify`. After the full Xcode toolchain became available, the current-source `dist/release/TetherPane.app` was rebuilt through the Xcode Release configuration and launched independently before the normal Xcode Debug development bundle was restored. Computer Use initially crashed while transforming the one-device accessibility tree. A temporary unchanged-`HEAD` comparison proved the tool and base app were healthy; a current-source probe narrowed the cycle to SwiftUI's labeled `GroupBox`. Replacing the three workspace group boxes with semantic native SwiftUI cards made the full final tree readable and kept the intended visual hierarchy.

Computer Use then read, interacted with, and captured the final app in these states:

- The exact local Release bundle showed the single secure Motorola workspace with Mirror, native mirror settings, route-specific Disconnect, and the Advanced inspector. Opening and closing Advanced worked in release mode; the app was then replaced by the verified development launch.
...
Dark appearance was visually verified against the real system setting. A recognized inert device-management fixture then applied process-local Light appearance, Reduce Motion, and Reduce Transparency overrides without changing persistent macOS settings or invoking ADB or scrcpy. Computer Use verified the complete Light workspace, navigated immediately between device, USB-C, and Wi-Fi states through the no-spring motion branch, and confirmed that the USB-C setup canvas replaced glass with an opaque semantic surface. Normal launches still read the real SwiftUI accessibility and appearance environment. Full Tab traversal remains unclaimed because the current macOS Full Keyboard Access setting disables it and changing that persistent setting requires explicit confirmation; keyboard commands, focusable native controls, and accessibility presentation branches are covered by the automated and UI-visible evidence above.
